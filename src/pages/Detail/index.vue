            ...
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd
                  ...
                  @click="changeSaleAttrValue(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)"
                >{{spuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @input="handlerSkuNum" />
                <a href="javascript:" class="plus" @click="skuNum++">+</a>
                <a href="javascript:" class="mins" @click="skuNum>1?skuNum--:skuNum=0">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="addCarShop">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    // 改变商品选中信息方法
    changeSaleAttrValue(SaleAttrValue, attr) {
      // 遍历数组清空所有ischecked属性
      attr.forEach(item => {
        item.isChecked = '0'
      })
      // 点击的元素设置isChecked属性为1
    ...
    async addCarShop() {
      // 发送请求将数据传至服务器
      try {
        // 将action中返回的Promise函数返回并用await解析
